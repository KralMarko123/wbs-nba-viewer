Cypress.Commands.add("checkDefaultState", () => {
	cy.get(".spinner").should("exist").get(".loading-text").should("have.text", "Getting Players");
...
Cypress.Commands.add("selectResultsCount", (count) => {
	...
	cy.get(".option").each((el) => (el.text().includes(count) ? cy.wrap(el).click() : null));
});
...
Cypress.Commands.add("checkPlayerInfoInDetailsPage", (playerName, dob, draftYear, team) => {
...
Cypress.Commands.add("checkNotFound", () => {
	...
	cy.get(".no-results-message").should("have.text", "Looks like there isn't anything here...");
});
