...
import SparqlService from "../api/SparqlFactory";
...
const Details = () => {
	...
	const [playerDetails, setPlayerDetails] = useState(myConstants.PLACEHOLDER_PLAYER_DETAILS);
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			await SparqlService.fetchPlayer(playerId).then((data) => {
				...
				window.sessionStorage.setItem(`${playerId}-details`, JSON.stringify(data[0]));
		...
		<div className="details page">
			<section className="player-container">
					...
					{isLoading ? (
						...
						<div className="player-details">
							...
							<div className="info">
								...
								<p className="info-text">
									{HelperFunctions.isWithLatinCharacters(playerDetails.team.name)}
								</p>
							</div>
							...
							<div className="info">
								...
								<p className="info-text">{playerDetails.draftYear || "Unknown"}</p>
							</div>
							...
							<div className="info">
								...
								<p className="info-text">
									{HelperFunctions.formatBasketballSuffix(playerDetails.position.name) || "Unknown"}
								</p>
							</div>
						</div>
					)}
				...
				<div className="bottom">
					...
					<button className="back-button" onClick={() => navigate("/wbs-nba-viewer")}>
						Back To Homepage
					</button>
				</div>
			</section>
